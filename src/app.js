...
function eixRenderExplore(body, D, verticals, horizontals, POIS){
  ...
  /* Per a cada punt, tria entre 4 diagonals possibles (amunt-dreta, avall-esquerra,
   * avall-dreta, amunt-esquerra) la que quedi més lluny d'altres punts i que no es
   * surti pel marge superior/inferior del mapa — en lloc d'una regla fixa. */
  var EIX_POI_DIRS={
    NE:{rot:-55, endAnchor:false, ox:6,  oy:3,  ux:0.574, uy:-0.819},
    SW:{rot:-55, endAnchor:true,  ox:-6, oy:-3, ux:-0.574,uy:0.819},
    SE:{rot:55,  endAnchor:false, ox:6,  oy:3,  ux:0.574, uy:0.819},
    NW:{rot:55,  endAnchor:true,  ox:-6, oy:-3, ux:-0.574,uy:-0.819}
  };
  placements.forEach(function(pl){
    var estLen=pl.p.name.length*5.2;
    var best=null, bestScore=Infinity;
    ['NE','SE','NW','SW'].forEach(function(dirName){
      var d=EIX_POI_DIRS[dirName];
      var ax=pl.x+d.ox, ay=pl.y+d.oy, fx=ax+d.ux*estLen, fy=ay+d.uy*estLen;
      var minY=Math.min(ay,fy), maxY=Math.max(ay,fy);
      var score=0;
      if(minY<6 || maxY>built.gridH-6) score+=1e6; /* mai sortir del mapa */
      placements.forEach(function(other){
        if(other===pl) return;
        for(var i=0;i<=6;i++){
          var t=i/6, px=ax+(fx-ax)*t, py=ay+(fy-ay)*t;
          var dist=Math.hypot(other.x-px, other.y-py);
          if(dist<16) score+=(16-dist);
        }
      });
      if(score<bestScore){ bestScore=score; best=dirName; }
    });
    pl.dir=best;
  });
  placements.forEach(function(pl){
    var p=pl.p, x=pl.x, y=pl.y, d=EIX_POI_DIRS[pl.dir];
    ...
    var lx=x+d.ox, ly=y+d.oy;
    lbl.setAttribute('x',lx); lbl.setAttribute('y',ly);
    lbl.setAttribute('text-anchor', d.endAnchor?'end':'start');
    lbl.setAttribute('transform','rotate('+d.rot+' '+lx+' '+ly+')');
